...
@register_criterion('confidence_boost_loss')
class ConfidenceBoostCriterion(FairseqCriterion):
    """Applies a entropy loss to contenders (past tokens and gold token)."""
    ...
    def forward(self, model, sample, reduce=True, compute_custom_metrics=True):
        ...
        with torch.no_grad():
            ...
            if self.candidate_type == 'prev_context':
                ...
                ctx_cands_ = (ctx_cands.tril() + self.padding_idx)
                ctx_cands_ = ctx_cands_ * ctx_cands_.triu(1)

                ctx_cands = ctx_cands.tril() + ctx_cands_
                cands_target = torch.zeros_like(lprobs).scatter_(1, ctx_cands, 1)
                
            else:
                raise NotImplementedError('candidate type %s' % self.candidate_type)

        
        candidate_entropy = (lprobs.exp() * lprobs)*cands_target
        custom_loss = -candidate_entropy.sum()
